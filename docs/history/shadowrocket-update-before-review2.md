...
この文書は過去の観測・判断の保存用であり、操作手順として使用しない。廃止済みの本文 pattern、conf 再取得、SHA のみのロールバックなど、現行実装と矛盾する指示を含む。現行手順は [更新・復旧ガイド](../shadowrocket_update.md)、保守方針は [CLAUDE.md](../../CLAUDE.md) を参照。
...
> **ログの扱い (プライバシー)**: Shadowrocket のログ DB は復号済みのフル URL
> (= 閲覧履歴そのもの) を含む。共有するのは**障害調査のスポット対応に限り**、
> 対象の症状が出た直後の短時間分だけにする。調査が終わったら削除する。
> ログの定期送信・巡回先リスト作成のためのログ提供は行わない。

## 今回 (2026-09-10 第10報) の反映手順 — モジュール更新だけで OK【緊急ロールバック】

第9報の広域 MITM で **Claude アプリが接続不能**になったため (インシデントレポート:
docs/incident-2026-09-10.md)、ユーザー指示により **3bc45ca 相当へロールバック**した。

復元される構成 (= 更新経路固定まで完了した時点の、最も安定していた構成):
- MITM は許可リスト (広告ドメイン + バッジ検証用 + trafficnews)。一般サイト・アプリは復号しない
- [Script] は「HTTPS 全ホストの文書 URL」(実際に処理されるのは MITM 対象のみ)
- newsdig は MITM 除外 + loader スタブリライトで壁抑止 (従来通り)
- 実行コードの SHA 固定 (8ab78744) は維持

唯一の設計差分 = **平文 HTTP 対策** (第10報):
平文 HTTP は MITM 許可リストが効かず requires-body が直撃するため
(blog.livedoor.jp の実機応答死)、pattern を「平文 HTTP は neverssl.com のみ許可」とした。
未知の平文 HTTP サイトは一切処理されない。

手順: **モジュール更新 → 接続 OFF/ON**。
確認: ①Claude アプリが復旧 ②LINE/決済等のアプリ正常 ③example.com バッジ
④blog.livedoor.jp の個人ブログ表示 ⑤trafficnews / newsdig 壁なし。

## (旧) 2026-09-10 第9報 — 広域 MITM + 除外 DB 方式【ロールバック済み・再使用禁止】

**方針転換 (ユーザー決定)**: 「初見サイトで注入系の快適機能が効かないのは厳しい。
数百単位のサイトを個別昇格するのは非現実的」との判断により、許可リスト方式 (第4〜8報) から
**広域 MITM + 除外 DB 方式**へ再転換した。

- **include**: 旧 conf と同じ TLD ワイルドカード群 (未知サイトも文書に注入が効く)
- **除外 (安全の生命線)**: AdGuard が保守する公開 DB [HttpsExclusions]
  (https://github.com/AdguardTeam/HttpsExclusions) の issues/android/sensitive/banks(.jp+国際
  ブランド) + 当プロジェクト実測分 = **438 ドメイン** (金融/決済/キャリア/メッセージング/
  AI アシスタント/EC/ストリーミング/ピンニングアプリ)
- **[Script] は広域文書 URL** だが実測で壊れるクラスを除外:
  close 応答死 (newsdig/livedoor/fnn/toyokeizai)・Shift_JIS (itmedia/kakaku/5ch/2chan)・
  SPA 敏感 (nicovideo)・/api/・/graphql/ パス
- すべて**モジュール側**の変更 (conf は無関係)。ロールバック = 前版モジュールに戻すだけ

手順: **モジュール更新 → 接続 OFF/ON**。
確認: ①example.com バッジ ②初見の一般サイトでもバッジ (#adkill を URL 末尾に付ける) が出る
③普段のアプリ (LINE/決済/銀行/Claude 等) が正常 ④trafficnews/newsdig/livedoor 正常。

**不調が出たときの対処 (この方式の運用ルール)**:
| 症状 | 対処 |
|---|---|
| アプリが通信エラー | `tools/mitm_exclusions_extra.txt` にドメイン追記 → `build_mitm_exclusions.py --module` → モジュール更新 |
| 特定サイトが白画面/読み込み失敗 | [Script] pattern のホスト除外に追加 → モジュール更新 |
| 特定サイトが文字化け | 同上 (Shift_JIS クラス) |
| 全面的に不調 | 前版モジュールへロールバック (git の1つ前の版 or 前リリースのアセット URL) |

## (旧) 2026-09-10 第8報の反映手順 — 第9報で方針転換により置き換え

第7報のホスト許可リスト化で「初見サイトでは注入系の快適機能 (空広告枠の折り畳み・
壁の掃除) が効かない」ため、**「巡回サイトの昇格」運用**を開始した:
初見サイト = フィルタのみ (安全側) / よく訪れるサイト = 安全確認バッテリーを通して
注入対象へ昇格。今回、実測検証の上で 5 サイトを昇格:
**jetstream.blog / rocketnews24.com / weathernews.jp / publickey1.jp / b.hatena.ne.jp**

### 昇格の条件 (すべて満たすこと)
1. `Connection: close` を明示しない (応答死クラス。toyokeizai は否決)
2. 施行 CSP を返さない (CSP 保持ガードで注入されないため無意味。ameblo/note/gigazine は否決)
3. **文書が UTF-8** (SR のバッファは非 UTF-8 を破壊しうる。Shift_JIS の itmedia/kakaku は否決)
4. WebSocket/SSE 等の長時間接続に依存しない
5. sr_emulator の注入ありエミュレーションで素の状態と同水準の描画
昇格作業: モジュールの hostname (%APPEND%) と [Script] pattern の両方にホストを追加。

手順: **モジュール更新 → 接続 OFF/ON**。
確認: 上記5サイトで空広告枠が畳まれる / example.com バッジ / 他サイトに変化なし。
不調が出たサイトはモジュールから外して再更新 (ロールバック)。

## (旧) 2026-09-10 第7報の反映手順 — モジュール更新だけで OK

http://blog.livedoor.jp/... (平文 HTTP の個人ブログ) が白画面になる実機事故。原因:
**平文 HTTP は MITM 許可リストと無関係に [Script] requires-body が適用される**ため、
旧 pattern (全ホストの文書 URL) では HTTP サイトがバッファリングに巻き込まれていた。
→ **pattern をホストごと許可リスト化** (バッジ検証用 + 壁対策サイトのみ)。
見知らぬサイトは HTTPS でも HTTP でも一切スクリプト処理を通らなくなった。

手順: **モジュール更新 → 接続 OFF/ON** のみ (第6報を実施済みであることが前提)。
確認: blog.livedoor.jp の個人ブログが表示される / example.com バッジ / trafficnews 正常。

## (旧) 2026-09-10 第6報の反映手順 — ⚠️ conf 更新 (最後の1回) + モジュール更新
