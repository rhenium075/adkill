...
以下の事故当時の経緯と矛盾する場合は、この節と docs/adversarial-hardening.md を優先する。
MITM 許可と本文処理許可は別々に管理する。tools/mitm_policy.json の審査済みパターンと、
conf/module の実効設定を tools/validate_config.js で照合する。本文処理は診断ホストの
ルートと trafficnews の公開トップ・記事・カテゴリに限定し、未知ホストやサブドメインへ
自動拡大しない。promote_batch.py の PASS は取得した一応答の結果であり、復号の許可ではない。

DOM 除去は名前と文言の両方で判定し、汎用ダイアログ・入力フォームを保護する。
iframe は URL のホストと /modal パスで判断する。ページの setTimeout は置換しない。
CSP は保持する。JP 変換ではネットワーク例外と重なるドメインを除外し、解釈できない
例外があれば更新を中止する。現在の配布リストの再生成・外部参照一式の固定は別途移行が必要。

エミュレータの標準動作はモジュールの固定 SHA のコードを読む。ADKILL_JS 指定は開発用で、
配布版の検証として扱わない。CDN への302は実行する。非文書が本文 pattern に一致した場合や
Connection:close 等の再現できない通信条件は非ゼロ終了して実機確認へ回す。
...
- adkill.js は施行 CSP のあるページでは注入を見送り、CSP を保持する。金融・決済系は復号対象に追加しない
